...
available for those orders, and are not used by mistake.
...
To allocate products or components from an |MO| to an |SO| or different |MO|, begin by navigating to
:menuselection:`Manufacturing --> Orders --> Manufacturing Orders`. Click :guilabel:`New` to create
a new |MO|.
...
the |MO|. This report lists open delivery orders or |MOs|, depending on the type of product produced
in the original |MO|.
...
.. example::
   An |MO| is created to produce three units of *wood*, which is used as a component for the rocking
   chair product. Clicking the :guilabel:`Allocation` smart button on the |MO| opens an allocation
   report that lists open rocking chair |MOs| that require one or more pieces of wood.
